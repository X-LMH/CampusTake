...
type Claims struct {
	...
	Role   enums.RoleType `json:"role,omitempty"`
...
func generateGenericToken(cfg JwtConfig, userID int64, role enums.RoleType, subject string, ttl time.Duration) (string, error) {
	now := time.Now()
	claims := Claims{
		UserID: userID,
		Role:   role,
...
func GenerateToken(cfg JwtConfig, userID int64, role enums.RoleType) (string, error) {
...
func ParseToken(tokenString string, secretKey string) (*Claims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &Claims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, errx2.NewParamError("token签名算法非法")
		}
	...
	if err != nil {
		if errors.Is(err, jwt.ErrTokenExpired) {
			return nil, errx2.NewCodeError(errx2.TokenExpireError, "凭证已过期")
		}
		return nil, errx2.NewCodeError(errx2.TokenInvalidError, "凭证无效")
	}

	claims, ok := token.Claims.(*Claims)
	if !ok || !token.Valid {
		return nil, errx2.NewCodeError(errx2.TokenInvalidError, "凭证校验失败")
	}
...
func ParseResetToken(tokenString string, secretKey string) (*Claims, error) {
	...
	if claims.Subject != "reset_password_token" {
		return nil, errx2.NewCodeError(errx2.TokenInvalidError, "凭证用途错误")
	}
	return claims, nil
}
